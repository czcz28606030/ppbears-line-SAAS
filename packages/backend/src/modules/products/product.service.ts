...
export class ProductService {
  ...
  formatProductsAsAiContext(products: Array<{ name: string; price: string; url: string; categories: string; phone_models?: string; }>): string {
    ...
    return `\n\n[產品索引搜尋結果 - 以下資料必須優先使用]\n${lines.join('\n')}\n[回覆規則] 1. 必須直接將購買連結以純文字 URL 格式完整回覆給客戶（例如：https://ppbears.com/...），不得使用 Markdown 的 [[url]] 雙括號格式。2. 不得叫客戶「自行搜尋」或「自行上網查找」。3. 若有找到相符產品，必定要附上連結。4. 絕對不可在回覆中列出商品價格或主動報價。5. 每次提供連結時，必須在句尾親切反問客戶確認（例如：「這個連結是您要找的型號嗎？若不是的話，請告訴我您的手機品牌與更完整的型號，我會再為您縮小範圍尋找！」）`;
  }
}
