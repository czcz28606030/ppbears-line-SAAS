...
## [v0.5.37] - 2026-04-23
### 🐛 修正
- 修正對話列表頁面 JSX 語法錯誤（重複 `<tr>` 標籤 + 缺少 `</tbody>`）導致 Build Error

## [v0.5.36] - 2026-04-23
### ✨ 功能
- **對話紀錄列表**：頂部新增「標籤篩選」深色下拉，可快速篩選出特定標籤的所有客戶對話
- **標籤篩選後批量回覆**：每行新增「回覆」按鈕，展開 inline 回覆面板，可選擇發送身份（人工客服/AI客服）逐一回覆，無需進入詳情頁
- 篩選時標題顯示「標籤「X」共 N 筆」及清除篩選按鈕

## [v0.5.35] - 2026-04-23
### ✨ 功能
- **對話詳情頁**：可直接在頁面回覆客戶訊息，支援 LINE / FB Messenger / WhatsApp 三大通路
- **發送身份切換**：每次送出前可選「人工客服」（橘色氣泡）或「AI 客服」（灰色氣泡），發送者 email 記錄於 metadata
- **Ctrl+Enter 快捷鍵**：輸入框按 Ctrl+Enter 快速送出
- 人工客服發出的訊息不顯示「修正回覆」按鈕（避免誤操作）

## [v0.5.34] - 2026-04-23
### ✨ 功能
- **受眾管理**：標籤下拉選單改為自製深色元件（原生 select 無法套用深色樣式）
- **對話紀錄列表**：新增標籤欄，可直接查看/新增/刪除客戶標籤，支援選取既有標籤下拉
- **對話詳情頁**：Info Card 新增標籤編輯區塊，閱讀對話時可即時貼標
- **標籤正規化**：修正 `iphone12PRO` 與 `iphone 12 PRO` 產生不同 tag 的 bug（數字-字母邊界自動插橫線）
- **iPhone 縮寫辨識**：新增 `I13`/`i14`/`I15PRO` 等縮寫自動展開為 `iphone 13`/`iphone 14`/`iphone 15 pro`

## [v0.5.33] - 2026-04-15
### 🐛 修正：行銷推廣「選擇標籤」下拉選單在深色模式下選項不可見

#### 根本原因
原本使用瀏覽器原生 `<select>` 元素。在深色主題下，下拉展開後瀏覽器強制以白色背景渲染 `<option>` 清單，而文字顏色與背景幾乎相同，導致選項看不清楚。

#### 修復方式
將原生 `<select>` 完全替換為自製的下拉選單 UI：
- 觸發按鈕維持與其他輸入欄一致的深色風格
- 展開後使用 `#1a1d2e` 深色背景 + `#d0d5ea` 淺色文字，對比清晰
- 已選中的選項高亮顯示（accent 色背景 + 白字 + 粗體）
- Hover 時背景淡藍色反饋
- 點擊選單外部自動收合

#### 修改檔案
- `packages/admin/src/app/(admin)/broadcast/page.tsx`：自訂下拉選單取代原生 `<select>`
